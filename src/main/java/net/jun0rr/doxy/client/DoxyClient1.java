...
public class DoxyClient1 {
  ...
  public DoxyClient1(DoxyEnvironment env) {
  ...
  public DoxyClient1 start() {
    ...
    this.server = TcpServer.open(setup, 1, env.configuration().getThreadPoolSize())
        ...
        .execute()
        .channel();
    httpGroup.scheduleAtFixedRate(httpPullService(), 0, 500, TimeUnit.MILLISECONDS);
    return this;
  }
  
  public DoxyClient1 stop() {
    this.service = false;
  ...
  private Supplier<HttpHandler> httpOutputJwtRequestHandler(String jwt) {
    return ()->x->{
      //System.out.println("--> httpOutputJwtRequestHandler");
  ...
  private Supplier<HttpHandler> httpNoContentHandler() {
    return ()->x->{
      ...
      System.out.println("[HTTP] Response received: " + x.response().status());
    ...
    return ()->x->{
      ...
      st.forEach(p->{
        ...
        if(ch != null) ch.eventChain()
            ...
            .onComplete(e->System.out.printf("[HTTP] Reponse received: %s -> %s (%d bytes)%n", e.channel().remoteHost(), p.channelID(), p.originalLength()))
            .execute();
      });
      ...
      if(ch != null) {
        ...
        ch.getObject().eventChain().write(
            ...
            .onComplete(e->System.out.println("[HTTP] PULL requested"))
            .execute();
      }
      return x.forward();
    };
  ...
  private void sendPullRequest() {
    ...
    if(ch != null) {
      ...
      ch.getObject().eventChain().write(
          HttpRequest.of(HttpVersion.HTTP_1_1, HttpMethod.GET, URI_PULL))
          .onComplete(e->System.out.println("[TCP] PULL requested"))
          .execute();
    }
  }
  ...
  private Supplier<Consumer<TcpExchange>> tcpConnectHandler() {
    return ()->x->{
      ...
      System.out.printf("[TCP] Client connected: %s -> %s%n", x.channel().remoteHost(), hash);
  ...
  private Supplier<TcpHandler> tcpForwardHandler() {
    return ()->x->{
      ...
      ch.getObject().eventChain()
          ...
          .onComplete(e->System.out.println("[HTTP] Packet pushed"))
          .execute();
      sendPullRequest();
      return x.empty();
    };
